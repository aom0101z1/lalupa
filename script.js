...
function openModal(id) {
    ...
    modalBody.innerHTML = `
        ...
        ${typeof isAdmin !== 'undefined' && isAdmin ? adminCasoBarHTML(caso) : ''}
...
// Barra solo-admin dentro del modal: desactivar/ocultar/publicar el caso sin buscarlo en el panel
function adminCasoBarHTML(caso) {
    const status = caseApprovals[caso.id];
    const etiqueta = status === 'rejected' ? 'Desactivado' : status === 'pending' ? 'Oculto' : 'Publicado';
    return `
        <div class="admin-caso-bar">
            <span class="admin-caso-estado"><i class="fas fa-user-shield"></i> Admin · EXP. #${caso.id} · ${etiqueta}</span>
            <div class="admin-caso-acciones">
                ${status !== 'rejected' ? `<button class="btn-block" onclick="setCaseStatusDesdeModal(${caso.id}, 'rejected')"><i class="fas fa-trash-alt"></i> Desactivar</button>` : ''}
                ${!status || status === 'approved' ? `<button class="btn-hide" onclick="setCaseStatusDesdeModal(${caso.id}, 'pending')"><i class="fas fa-eye-slash"></i> Ocultar</button>` : ''}
                ${status === 'rejected' || status === 'pending' ? `<button class="btn-unblock" onclick="setCaseStatusDesdeModal(${caso.id}, 'approved')"><i class="fas fa-eye"></i> Publicar</button>` : ''}
            </div>
        </div>`;
}

async function setCaseStatusDesdeModal(caseId, status) {
    if (status === 'rejected' && !confirm('¿Desactivar este caso? Dejará de mostrarse en el sitio (se puede reactivar desde Panel Admin → Casos Publicados).')) return;
    await setCaseStatus(caseId, status);
    if (status === 'rejected') closeModal();
    else openModal(caseId);
}
...
function renderPendingCasesTable() {
    ...
    const autoGeneratedCases = data.casos.filter(c => c.auto_generated || caseApprovals[c.id]);
